$(document).ready(async function () {
  ...
  // ✅ 유저 기본 정보 로드
  try {
    const res = await apiRequest(`/game/${gameId}/user/${userId}`, "GET", null, token);
    if (res.success && res.data) {
      ...
      $("#level").val(u.level);
      $("#status").val(u.status);
      $("#createdAt").val(u.createdAt ? u.createdAt.split("T")[0] : "-");
    }
  } catch {
  ...
  $("#btnUpdateUser").click(async function () {
    ...
    try {
      const res = await apiRequest(`/game/${gameId}/user/${userId}`, "PATCH", body, token);
  ...
  $("#btnDeleteUser").click(async function () {
    ...
    try {
      const res = await apiRequest(`/game/${gameId}/user/${userId}`, "DELETE", null, token);
      if (res.success) {
        alert("삭제 완료");
        location.href = "game-users.html";
      } else alert("삭제 실패");
